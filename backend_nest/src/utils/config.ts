...
export const connectDB = (
  ...
  const dbUserame = configService.get<string>('MONGODB_USERNAME');
  const dbName = configService.get<string>('MONGODB_DATABASE_NAME');
  const mongodbUri = `mongodb+srv://${dbUserame}:${dbPassword}@cluster0.ybhszs8.mongodb.net/${dbName}?retryWrites=true&w=majority`;
  return {
    uri: mongodbUri,
    autoIndex: false,
  };
